      ...
      <div className="p-2">
        ...
        {balance >= state.amount ? (
          <div className="text-orange-600 text-center animate-pulse duration-75">
            <span className="font-extrabold">Warning!</span> ${state.amount}{" "}
            will be deducted from your wallet.
          </div>
        ) : (
          <div className="text-orange-600 text-center animate-pulse duration-75">
            <span className="font-extrabold">Warning!</span> You have to pay $
            {state.amount} manualy.
          </div>
        )}
  ...
  return (
    <Form onSubmit={onSubmit}>
      <div className={style("p-6").card()}>
        ...
        <div
          className={style("md:gap-6 gap-0 grid md:grid-cols-2 grid-cols-1")}
        >
          <div className="pt-6">
            <TextBox
              required
        ...
        <div
          className={style(
            "text-xs font-extrabold text-slate-900 animate-pulse"
          )}
        >
          N.B.: -k
          {balance >= state.amount
        ...
        <div className="text-red-600 text-center pt-6">{error}</div>
        <div className="flex justify-center pt-6">
          <Button
            disabled={busy}
            color={balance >= state.amount ? "green" : "orange"}
            type="submit"
          >
